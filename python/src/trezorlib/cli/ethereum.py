...
def _get_ethereum_definitions(
    ...
    if count_of_options_used > 1:
        raise click.ClickException("More than one mutually exclusive option for definitions was used.")

    defs = ethereum.messages.EthereumEncodedDefinitions()
    if definitions_dir is not None:
        return ethereum.get_definitions_from_dir(
            definitions_dir,
            chain_id,
            slip44,
            token_address,
        ...
        if chain_id is not None or slip44 is not None:
            defs.encoded_network = ethereum.download_network_definition(chain_id, slip44)
...
def download_definitions(outdir: pathlib.Path) -> str:
    pass
